...
class TextArea {
    ...
    public maxSymbols: HTMLElement;
    public buttonSend: HTMLButtonElement;
    public warning: HTMLElement;
    constructor() {
        this.input = document.getElementById('comment-input') as HTMLTextAreaElement;
        this.name = document.getElementById('name') as HTMLElement;
        this.maxSymbols = document.getElementById('max_symbols') as HTMLElement;
        this.buttonSend = document.getElementById('send') as HTMLButtonElement;
        this.warning = document.getElementById('warning') as HTMLElement;

        if (window.innerWidth <= 575) {
            this.avatar = document.querySelector('.avatar-mobile') as HTMLImageElement;
            this.spinner = document.querySelector('.spinner-mobile') as HTMLElement;
        } else {
            this.avatar = document.querySelector('.avatar') as HTMLImageElement;
            this.spinner = document.querySelector('.spinner') as HTMLElement;
        }
    }

    // Запрос данных пользователя
    async fetchData(): Promise<RandomUserResponse | undefined> {
    ...
    async init(): Promise<void> {
        ...
        if (data && data.results.length > 0) {
            ...
            if (user) this.avatar.src = user.picture.thumbnail;    
            
            localStorage.setItem('currentUserName', fullName);
            if (user) localStorage.setItem('currentAvatarSrc', user.picture.thumbnail);
