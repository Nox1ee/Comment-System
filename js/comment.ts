...
class Comment {
    public comments: CommentItem[]; // Хранилище комментариев
    public responses: ResponseItem[]; // Хранилище ответов
    public commentsContainer: HTMLElement | null; // Контейнер для комментариев

    public isResponse: boolean; // Если false, публикуется комментарий, иначе ответ


    public id: number; // id комментария на который будет дан ответ
    public commentName: string; // Имя того, на чей комментарий будет дан ответ
    constructor() {
    ...
    displayComments(): void {
        ...
        commentsToDisplay.forEach(comment => {
            ...
            const favBtn = commentDiv.querySelector('.comment__like-btn') as HTMLButtonElement | null;
            commentDiv.setAttribute('isFav', comment.isFavorite ? 'yes' : 'no');
            if (favBtn) {
                favBtn.onclick = () => this.toggleFavorite(comment.id);
            }

            const voteMinus = commentDiv.querySelector('.rating__minus') as HTMLButtonElement | null;
            if (voteMinus) {
                voteMinus.onclick = () => this.changeRating(comment.id, 'minus');
            }

            const votePlus = commentDiv.querySelector('.rating__plus') as HTMLButtonElement | null;
            if (votePlus) {
                votePlus.onclick = () => this.changeRating(comment.id, 'plus');
            }

            const responseBtn = commentDiv.querySelector('.comment__response-btn') as HTMLButtonElement | null;
            if (responseBtn) {
                responseBtn.onclick = () => {
                    this.id = comment.id;
        ...
        const commentsCountContainer = document.getElementById('commentsBtn') as HTMLElement | null;
    ...
    displayResponses() {
        ...
        responsesToDisplay.forEach(response => {
            ...
            const favBtn = responseDiv.querySelector('.comment__like-btn') as HTMLButtonElement | null;
            responseDiv.setAttribute('isFav', response.isFavorite ? 'yes' : 'no');
            if (favBtn) {
                favBtn.onclick = () => this.toggleFavorite(response.id);
            }

            const voteMinus = responseDiv.querySelector('.rating__minus') as HTMLButtonElement | null;
            if (voteMinus) {
                voteMinus.onclick = () => this.changeRating(response.id, 'minus');
            }

            const votePlus = responseDiv.querySelector('.rating__plus') as HTMLButtonElement | null;
            if (votePlus) {
    ...
    createComment(input: string):void {
        if (input.trim()) {
    ...
    // Добавление ответов
    createResponse(input: string, id:number, name:string):void {
        if (input.trim()) {
    ...
    // Измениение рейтинга 
    changeRating(id:number, symbol:string) {
    ...
    drawRating():void {
        const numbersOfRating = document.querySelectorAll<HTMLElement>('.vote-rating');
        numbersOfRating.forEach((el: HTMLElement) => {
